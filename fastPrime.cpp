...
uint64_t find_prime(uint32_t* arr_prime, uint64_t start, uint64_t n, bool is_count)
{
	...
	if(!is_count) return 0;
    for (i = start; i <= n; i += 2)//소수만 골라 내는 작업을 해줍니다.
    {
...
uint64_t  find_prime_max(uint32_t* arr_prime, uint64_t start, uint64_t n)
{
    uint64_t i, j, k, cnt = 0;
    uint64_t last = static_cast<uint64_t>(sqrt(static_cast<double>(n)));
    ...
    for (i = start; i <= n;)//소수만 골라 내는 작업을 해줍니다.
    {
        idx = (i >> shift_value) - start_index;
        bit = i & mask;
        bit = 1ULL << (bit >> 1);
...
void get_cached_data(uint64_t n)
{
    ...
    while(true)
    {
		...
		find_prime(arr_prime, start, last, false);//체로 걸러서
...
void run_find_prime_max(uint64_t __start, uint64_t start, uint64_t last, uint64_t end, auto startTime)
{
    uint32_t* arr_prime;
    uint64_t i, cnt = 0;
...
void run_find_prime()
{
	...
	while(true)
	{
		...
		if(end > UINT64_MAX - 10000000000)  {
		    run_find_prime_max(__start, start, last, end, startTime);
		    continue;
		} 
	    auto t1 = chrono::steady_clock::now();
	    while(true)
	    {
			...
			cnt += find_prime(arr_prime, start, last, true);
